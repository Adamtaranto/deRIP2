...
keys** (or the Prev/Next buttons). Every figure is embedded as inline SVG, so
the file has no external assets — it can be emailed or archived as one file, and
the figures stay vector (zoom in, or lift one straight into a manuscript).

Each sequence gets a panel with four parts.

### Alignment row

The sequence's alignment row, with its RIP-related sites highlighted over a
neutral grey backdrop and, beneath it, the reconstructed deRIP'd base per
column. Blue marks a RIP **product** (the deaminated base), orange the surviving
**substrate**; a third hue marks non-RIP deamination when `--reaminate` is set.
...
product (`TA`) or the surviving substrate (`CA` on the forward strand, `TG` on
the reverse). Because a base is either product or substrate, each column shows
at most one bar — so the strip reads as a clean yes/no map of where RIP struck
this copy.
...
### Per-sequence mutation spectrum

The sequence's SBS-96 trinucleotide mutation spectrum, measured against the
reconstructed ancestor. This is exactly the
[mutation-spectrum](mutation-spectra.md) analysis restricted to one row
(`calculate_spectra(partition_by='row')`), so RIP's `C>T`/`G>A` signature in
`CpA` context shows up per sequence.

### Summary statistics

That sequence's row from the per-sequence statistics table — GC, CRI and its
components, RSI and its strand proportions, RIP event counts and significance —
the same values written by `--stats-out`.
...
### Gene-effect panels in the per-sequence report

Each annotated sequence's panel gains a **gene effects** table. deRIP2 assembles
the sequence's CDS in transcription order (reverse-complementing minus-strand
genes and joining exons), translates it against the reconstructed ancestor, and
reports:
...
(`per_sequence_strand_bias`, `sequence_row_strip`).
